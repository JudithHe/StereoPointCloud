...
def show_feature_depth(image_left_resized, image_right_resized, reference_left, reference_right, depth, w=None, h=None):
    ...
    for i in range(len(reference_left)):
        ...
        draw_circle(image_left_resized, x, y, 3, depth_to_color(depth[i]))

        [x, y] = reference_right[i]
        draw_circle(image_right_resized, x, y, 3, depth_to_color(depth[i]))
...
def extrapolate_depth(image_left_resized, image_right_resized, reference_left, reference_right, depth):
    ...
    w = image_left_resized.shape[1]
    h = image_right_resized.shape[0]
    ...
    for i in range(0, h, step):
        ...
        for j in range(0, w, step):
            ...
            image_left_output += [[j, i, d, image_left_resized[i][j][2], image_left_resized[i][j][1],
                                   image_left_resized[i][j][0]]]

    return [image_left_output]
...
def generate_output_file(image_output):
    print("Writing results into file...")

    file = open("points.txt", "w")

    file.write(str(len(image_output)) + "\n")
    for i in range(len(image_output)):
        feature = ""
        for j in image_output[i]:
            feature += str(j) + " "
        file.write(feature + "\n")
...
def main():
    ...
    image_left_raw = cv2.resize(image_left_raw, (RESIZE_WIDTH, RESIZE_HEIGHT))
    image_right_raw = cv2.resize(image_right_raw, (RESIZE_WIDTH, RESIZE_HEIGHT))
    ...
    [image_left_output] = extrapolate_depth(image_left_raw, image_right_raw, reference_left,
                                                       reference_right, depth)

    # Writing the final result into a file
    generate_output_file(image_left_output)
